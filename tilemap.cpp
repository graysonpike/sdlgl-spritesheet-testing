...
Tilemap::Tilemap(int scale, int width, int height, Tileset tileset) : scale(scale), width(width), height(height), tileset(tileset) {}

int Tilemap::coords_to_index(int x, int y) {
    return x + (y * width);
}
...
void Tilemap::draw(int layer) {
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            tileset.draw_tile(i * scale, j * scale, layers[layer][coords_to_index(i, j)]);
        }
    }
}